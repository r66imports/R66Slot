 ...
 */
function openR66EditorPanel(context, document) {
    ...
    currentPanel = vscode.window.createWebviewPanel('r66Editor', 'R66 Visual Editor', vscode.ViewColumn.Beside, {
        enableScripts: true,
 ...
 */
async function handleUpdateY(sectionId, newY, deltaY) {
    const editor = vscode.window.activeTextEditor;
    if (!editor)
        return;
 ...
 */
async function handleUpdatePosition(sectionId, x, y) {
    const editor = vscode.window.activeTextEditor;
    if (!editor)
        return;
    const document = editor.document;
 ...
 */
async function handleUpdateSize(sectionId, width, height) {
    const editor = vscode.window.activeTextEditor;
    if (!editor)
        return;
    const document = editor.document;
 ...
 */
function parseSectionsFromDocument() {
    const editor = vscode.window.activeTextEditor;
    if (!editor)
        return [];
 ...
 */
function findSectionInDocument(text, sectionId) {
 ...
 */
function findPropertyInSection(text, sectionStart, sectionEnd, propertyName) {
 ...
 */
function findObjectEnd(text, fromIndex) {
    let depth = 0;
...
// ═══════════════════════════════════════════════════════════════════════════
function getWebviewContent(document) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
