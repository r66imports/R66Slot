    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {allImages.map((img, idx) => (
                <button key={idx} onClick={() => setActiveImg(idx)}
                  className={`w-16 h-16 rounded-lg overflow-hidden border-2 flex-shrink-0 transition-all ${activeImg === idx ? 'border-red-600' : 'border-gray-200 hover:border-gray-400'}`}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          {product.sku && <p className="text-xs text-gray-400 font-mono mt-3">{product.sku}</p>}
        </div>

        {/* ── Right: Info ── */}
        <div className="flex flex-col gap-4">
          {product.brand && <p className="text-sm font-bold uppercase tracking-widest text-gray-500">{product.brand}</p>}
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 leading-tight">{product.title}</h1>
          {product.sku && <p className="text-sm text-gray-500">SKU: <span className="font-mono font-semibold text-gray-700">{product.sku}</span></p>}
          ...
          {!outOfStock && !isPreOrder && product.quantity > 0 && (
            <p className="text-sm font-medium text-green-600">{product.quantity} available</p>
          )}
            ...
            <div className="flex flex-col gap-3">
              {!isPreOrder && (
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 border border-gray-300 rounded-lg overflow-hidden">
                    ...
                    <button onClick={() => setQty((q) => Math.min(isFinite(stockLimit) ? stockLimit : q + 1, q + 1))}
              ...
              {isPreOrder ? (
                ...
                <button onClick={handleAddToCart}
                  className={`w-full font-bold py-3 px-6 rounded-lg text-base transition-all duration-200 ${added ? 'bg-green-600 text-white scale-95' : 'bg-red-600 text-white hover:bg-red-700'}`}>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {product.carClass && <span className="px-3 py-1 bg-black text-white text-xs font-bold rounded-full">{product.carClass}</span>}
            {product.scale && <span className="px-3 py-1 bg-gray-100 text-gray-700 text-xs font-semibold rounded-full">Scale {product.scale}</span>}
            {product.productType && <span className="px-3 py-1 bg-gray-100 text-gray-700 text-xs font-semibold rounded-full">{product.productType}</span>}
          </div>
