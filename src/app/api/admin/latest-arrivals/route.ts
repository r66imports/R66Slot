...
export async function POST(req: NextRequest) {
  try {
    ...
    // When an array is sent (worksheet button), REPLACE the entire blob so stale/excluded
    // items (e.g. previously-added Chasecar entries) are always removed.
    // When a single item is sent, upsert into the existing list.
    let arr: ArrivalItem[] = []
    if (!isArray) {
