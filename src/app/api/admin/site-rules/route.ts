...
const DEFAULT_RULES: SiteRule[] = [
  ...
  },
  {
    ...
    description: 'Automatically adjusts inventory when documents are created, edited, rejected or deleted. Flow: Quote created \u2192 no stock impact. Sales Order created \u2192 stock reserved (deducted immediately, pending delivery). Invoice created \u2192 stock confirmed as sold (deducted). Line items edited on active SO/Invoice \u2192 old quantities restored, new quantities deducted. ARCHIVING MOVES NO STOCK. Archive is a view filter: it clears a spent quote or a finished invoice off the active list to keep the working list fast, and carries no business meaning on any document type, paid or unpaid. An archived document is INERT \u2014 no later save deducts or restores it, and returning it to active moves nothing either. TO UNDO A SALE, REJECT THE DOCUMENT OR DELETE IT. Both restore stock deliberately, and a rejection restores from any status, including a document that was archived first. Use the "Sync Inventory" button on the Orders page to apply deductions to historical invoices and sales orders that predate this rule; it skips archived and rejected documents.',
    active: true,
  ...
  {
    id: 'specials_slider',
    name: 'Rule 55 — Latest Arrivals & Specials Sliders: Product Status Toggles',
    description: `Latest Arrivals and Specials are two homepage product sliders, each on its own separate list — they never interfere. Flow: Product Edit → Product status → 🆕 Latest Arrivals or 🏷️ Specials toggle → saves immediately (not on Save) → the product is upserted into its blob (data/latest-arrivals.json or data/specials.json: sku, title, image, price, quantity, productId, addedAt) via /api/admin/latest-arrivals or /api/admin/specials → switching the toggle OFF removes that entry. Two ways in for Latest Arrivals: the Product Edit toggle, and the Pre-Order Dashboard's “🆕 Send to Latest Arrivals” button, which resolves the real product row by SKU so the card's quantity is live Inventory stock and NOT the dashboard's own reservation-slot count. The dashboard tracks its own sentToLatestArrivals flag so the button reads “🆕 In Latest Arrivals” after a reload — that field must stay in the PATCH allowedFields whitelist in /api/admin/preorder-dashboard/[id], or the flag saves and silently vanishes. Specials has one way in only, the Product Edit toggle. The Worksheet adds to neither. An item sent to Latest Arrivals has landed, so it is no longer a pre-order and leaves the Pre-Order Dashboard list — hidden, never deleted: its customers and document links stay, a search still finds it, and the toolbar's “🆕 Show arrived (N)” button brings the hidden items back. The Page Editor has matching "Latest Arrivals" and "Specials" slider elements that read /api/latest-arrivals and /api/specials. Both merge live stock quantities from the products table; /api/specials also re-reads title, price, Discount % and image, so the card always reflects the current product. Each element shows exactly the products with its toggle switched ON — there is no manual product picker. Card images are never trusted from the blob snapshot alone: flipping the toggle before an image finishes uploading stores an empty imageUrl, so /api/specials resolves the image from the product's own image_url / images[0] when the stored one is blank or a data: URL. Deleting a product does not leave a phantom Specials card — /api/specials drops any entry whose SKU no longer matches a product row — but the blob entry is only removed by the toggle or the admin DELETE, so restoring a product under the same SKU brings its special back. COST PRIVACY: on R66Slot compareAtPrice is the internal Average Cost, not a "was" price — the toggles never send it and all three public feeds strip it, so it can never reach the storefront. Differences between the two, both intentional: (1) Days Visible defaults to 30 on Latest Arrivals and 0 on Specials, where 0 means never expire — a special stays up until the toggle is switched off; (2) Specials cards show the retail price struck through, the discount % and the discounted price (Rule 56); Latest Arrivals cards show a single plain price. Chasecar items cannot be added to Latest Arrivals (or Landing Soon, Rule 57); Specials has no such exclusion. LOOPING: with Slider Auto Speed above 0, a slider only glides and loops once one set of cards is wider than its row (re-measured on resize). Until then it sits still and shows each card once — it never renders the duplicate set that the seamless loop needs, so a short slider can never show a card twice. All three sliders render through one shared component (ProductSliderLive on the live site, one shared case in the editor canvas) so their layout, colours and settings can never diverge — any change to one must be made in the shared component, and each element type must stay registered in all six places: lib/pages/schema.ts, the editor COMPONENT_DEFAULTS, the r66-editor sidebar library, rendered-component.tsx, editor-properties-panel.tsx and component-renderer.tsx.`,
    active: true,
    appliesTo: ['Page Editor', 'Products', 'Online Store'],
    category: 'Page Editor',
  },
  {
    id: 'product_retail_discount',
    name: 'Rule 56 — Product Retail Discount %',
    description: `Product Edit → Pricing (Rand) has a Discount field: a % box and the resulting rand price, two-way bound (type a % and the price fills in, or type a price and the % back-fills). The Price field above it ALWAYS stays the full retail figure — it is never overwritten by the discount. The discounted price is derived as price × (1 − discountPct/100), so the original retail is never lost and the discount can be cleared back to 0 at any time. Stored in products.discount_pct (self-healing ALTER TABLE on product save, and once per process in /api/specials so the slider works before any product is saved). handleSave and doAutosave both always send discountPct as a number, never null, because the PUT route COALESCEs nulls and a null would silently keep the previous discount. Where it shows: the Specials slider card renders three lines — retail price struck through, the discount % (e.g. -10%), and the discounted price. /api/specials re-reads price and discount_pct from the products table on every request, so changing a product's Discount % updates the slider immediately with no need to toggle Specials off and on. A Specials card with no Discount % shows a single plain price — there is no compareAtPrice fallback on R66Slot (compareAtPrice is the internal Average Cost). The Specials element's Colours section has its own Discount % and Discounted Price pickers (defaults #f59e0b and #22c55e). IMPORTANT SCOPE: the discount is display-only on the Specials slider — the store product page, product grid, cart, checkout and invoices all still use the full retail price. Extending the discount to those flows is a separate change.`,
    active: true,
    appliesTo: ['Products', 'Page Editor', 'Online Store'],
    category: 'Page Editor',
  },
  {
    id: 'landing_soon_slider',
    name: 'Rule 57 — Landing Soon Slider & Product Status Toggle',
    description: `Landing Soon is the pre-arrival slider — stock that is on its way but has not landed yet. It works like Latest Arrivals (Rule 55) on its own separate list; Latest Arrivals, Landing Soon and Specials are three independent blobs that never interfere. Two ways in, both writing data/landing-soon.json via /api/admin/landing-soon: (1) Product Edit → Product status → 🛬 Landing Soon toggle → saves immediately (not on Save) → upserts the product → switching OFF removes that entry; (2) Pre-Order Dashboard item → “🛬 Send to Landing Soon” button, which snapshots the dashboard's remaining slots as the card qty (the item has not landed, so it has no Inventory stock) and sets sentToLandingSoon on the card so the button reads “🛬 In Landing Soon” after a reload — that field must stay in the PATCH allowedFields whitelist in /api/admin/preorder-dashboard/[id]. The Page Editor "Landing Soon" slider element reads /api/landing-soon, which merges live price and image from the products table. QUANTITY IS THE EXCEPTION: where the SKU is on the Pre-Order Dashboard, the card's qty is the dashboard's remaining slots, NOT products.quantity. The formula is the one the dashboard prints as "(N in stock)" — minOrderQty > 0 ? max(0, minOrderQty − total reserved customer qty) : extraQty — duplicated in dashboardAvailableQty() in /api/landing-soon and the inStock const in admin/preorder-dashboard/[supplier]/page.tsx, so the two must stay in step. Pre-arrival stock has no inventory yet, so reading products.quantity would show every card as sold out while the dashboard still had slots open. Where the same SKU appears on more than one dashboard item, the highest availability wins. An entry with a dashboard match but no product row is kept (pre-order stock legitimately has no product row until the shipment lands); only an entry missing from BOTH sides is dropped. A SKU not on the dashboard falls back to products.quantity. Days Visible defaults to 0 (never expire) — incoming stock stays up until the toggle is switched off, usually once the shipment lands and it moves across to Latest Arrivals. Cards show a single plain price with no discount lines. The Chasecar exclusion applies to Landing Soon — a pre-arrival reveal is exactly the exposure it exists to prevent. Landing Soon shares ProductSliderLive, the looping rule and the six-place registration rule in Rule 55.`,
    active: true,
    appliesTo: ['Page Editor', 'Products', 'Pre-Order Dashboard', 'Online Store'],
    category: 'Page Editor',
  },
  {
    id: 'slider_card_sizes',
    name: 'Rule 58 — Slider Card Sizes (Latest Arrivals, Landing Soon, Specials)',
    description: `All three product sliders share one Card Size control in the Page Editor properties panel, with four options: Small (160px wide / 140px image), Medium (220px / 200px), Large (280px / 260px) and Extra Large (360px / 340px). The size map is duplicated in exactly two places and the two must stay in step or the editor canvas will preview a different size to the live site: cardSizeMap in ProductSliderLive (component-renderer.tsx) and the inline laCardW / laCardH maps in the shared slider case in rendered-component.tsx. The live cardSizeMap falls back to Small on an unrecognised value rather than returning undefined, so a page saved with a card size that was later removed still renders instead of crashing on a missing width. Card size affects the card width and the image height only — font sizes are set independently in the panel's Font Sizes section, so bumping to Extra Large does not scale the title or price text.`,
    active: true,
  ...
  },
  {
    ...
    description: `A Supplier Pre Order is a client asking us to order something from a supplier. It is not stock and must never behave like stock. Submitting, editing, merging or archiving one creates no product, writes nothing to products.quantity, raises no reservation and produces no stock-log movement. The Supplier Catalogue behind it (data/supplier-catalogue.json) is likewise a list of what a supplier can sell us, not what we own — a SKU can sit on the sheet forever without ever having been a product, which is the point: clients request items we have never carried. Stock only ever appears the ordinary way, when the goods are merged into a Supplier Order, shipped, and landed through the Worksheet. A client may also type in a SKU that is not on the sheet at all; those lines are flagged isNewSku and carry no price until someone costs them by hand. TWO QTY COLUMNS SIT SIDE BY SIDE on both the client sheet and the admin request view, and neither is a reservation: “In Stock” is products.quantity, what is on the shelf right now; “Qty Ordered” is qty already PLACED WITH THE SUPPLIER — lines on pre-orders at status ordered, deposit-paid or paid. A request still being gathered (submitted, reviewed, quoted) counts as 0, deliberately: it may never be bought, and counting it would tell both sides stock is coming when nothing has been ordered. Rejected lines and binned requests never count. The figure is SCOPED BY VIEWER — admin sees the total across every client, a client sees only their own qty on order, so no client ever learns what another has asked for. Both columns are derived on read (getOnOrderQtyBySku in lib/supplier-catalogue.ts) and never written back into the request.`,
    active: true,
  ...
  },
  {
    ...
    description: `Every estimate on a Supplier Pre Order comes from one function, calcEstRetailZAR in src/lib/preorder-pricing.ts, so the client sheet, the admin page and the quote can never disagree. The formula is landed = wholesale × exRate × (1 + (shipping% + customs% + handling%)/100), then × landedMultiplier, then retail = landed × (1 + markup%) × (1 + vat%). It is seeded to reproduce the Business Calculator's Spare Parts mode exactly: that calculator carries shipping and customs as one 45% figure, split here into 25% shipping + 20% customs so the two can be tuned separately — 25 + 20 must keep summing to whatever the calculator uses or the two will quote different prices for the same item. 100 EUR at R20.00 gives R4 335.50 in both. landedMultiplier is 1 for BOTH accounts: the Worksheet's calcEntityFinalLanded adds 15% on landed for Route 66 Imports as an internal inter-company figure, but a client is quoted against the Spare Parts Calculator, which has no such uplift, so the client estimate does not apply it either. The field is kept so it can be raised on the Costing Accounts panel without a deploy. The percentages come from the costing account (data/costing-accounts.json), chosen per supplier and overridable by an admin per order; the client never sees or picks the account, the wholesale price or the percentages. The exchange rate is live and re-read on every page load, so an estimate moves until it is fixed — which is why every client-facing price carries the disclaimer that estimated retail prices fluctuate with the rate of exchange. Once a Worksheet Final covers a SKU, the line's finalExRate and derived shipping/customs/handling percentages are copied onto it and priceLocked is set; a locked line is never re-priced against the live rate again. TWO PRICES SIT SIDE BY SIDE — on the client sheet, on the client's downloaded PDF and on the admin request view — and they are not the same number: “Retail” is products.price, what the SKU sells for on the site today, and reads “—” for a SKU we have never carried; “Est. Retail” is this rule's calculation off the wholesale price and reads “On request” when no wholesale is known. They differ legitimately — the shelf price was set against an older rate and landing, the estimate floats with today's (Rule 63) — so neither is corrected to match the other. Retail is carried on MergedItem.retailZAR, taken from the product row only; the wholesale sheet has no shelf price, and on R66Slot compareAtPrice is Average Cost and never leaves the admin. On the admin line table and in the PDF the same figure comes from getSkuInfo, which reads products.price and nothing a client may not see. WHERE THE WHOLESALE PRICE COMES FROM on the client sheet, in order: the supplier catalogue row's own wholesalePrice, then the SKU's entry in the inventory pricelist (data/inventory-pricelists.json) through estimateFor in lib/preorder-estimate.ts — the same estimator the Pre-Order Dashboard and the booking widget use, so a client cannot be quoted one number here and another there. Only when NEITHER knows a wholesale price does Est. Retail fall back to pre_order_price, then products.price, which is why an item with no wholesale on file reads the same in both columns: that is a gap in the pricelist, not a calculation. A SKU carrying a landed costPerItem has arrived and its price is settled, so it stops floating (Rule 63).`,
    active: true,
  ...
  },
  {
    ...
    description: `The client sheet is brand-first: a client picks one or more brands and gets that brand's SKUs in natural alphanumeric order (so RS9, RS10, RS100 — never the plain string sort that puts RS100 before RS9). What they can order is TWO sources merged on SKU, in src/lib/supplier-catalogue.ts: every non-archived product in Inventory that carries a brand, plus the wholesale catalogue, which adds SKUs we have never stocked. The catalogue wins on overlap, because a wholesale price entered or imported there is more current than anything Inventory can say. This is why picking a brand lists its SKUs without anyone maintaining a second list by hand — and listing a product here does not make the request inventory; a pre-order still reserves and deducts nothing (Rule 59). Pricing follows the same precedence: a known wholesale price goes through the costing calculator; an Inventory item without one keeps the retail we already sell it for (pre_order_price ahead of price, since that field exists for goods bought ahead); neither means the client sees "On request" and an admin prices it by hand. A line priced that second way must not be recalculated on later reads — lineEstRetailZAR keeps the stored figure when the calculation returns 0, or every reload would blank the price. Search runs server-side over SKU and description and works with NO brand selected, so a client who knows the SKU need not guess its brand first. The brand resolves the supplier, and through it the currency and costing account: picking Sideways reaches Slotcar Boutique's. That mapping lives in the brands[] array on the supplier contact, maintained on the Supplier Catalogue page, falling back to the product's own supplier field. The one exception to the pricing precedence above is a local supplier: a ZAR wholesale price skips the calculator and keeps the Inventory retail (Rule 65).`,
    active: true,
  ...
  },
  {
    ...
    description: `An estimated retail price is never a stored number. It is DERIVED, on every read, from the supplier's wholesale price in the supplier's own currency multiplied by the live exchange rate, and it moves as that rate moves — which is why every client-facing estimate carries the disclaimer that estimated retail prices fluctuate with the rate of exchange. The wholesale price is the single source of truth and it must carry its currency: a price without a unit is not a price, and inferring one from the supplier at read time (or worse, defaulting to ZAR) is what once made a €33.90 Sideways price read as R33.90 and every margin built on it wrong. So the currency is stored on the price list entry itself, stamped when written, and changing a supplier's currency later cannot silently re-interpret prices already quoted. It follows that a converted ZAR figure must never be frozen into a product field as a substitute for the estimate — the moment it is stored it stops floating, and it is stale by the next rate change. costPerItem is the one exception and it is not an estimate: it is the REAL landed Rand cost of goods actually received, written by the Worksheet when a shipment lands. AN ESTIMATE DOES NOT STOP FLOATING WHEN THE SHIPMENT LANDS. It floats for as long as a wholesale price exists, arrived or not, because it answers what the NEXT shipment would cost — the rand keeps moving after goods arrive, and a client quoted off a landed SKU was being quoted at whatever the rate did months ago. This reverses the earlier lock-on-landing behaviour, which only existed because a landed price was a historical fact with nowhere else to live. It now has somewhere: ESTIMATE RETAIL PRICE FROM WORKSHEET on the product Pricing card (worksheet_est_retail) holds the figure a Worksheet import produced, stored with the exchange rate, currency and sheet date it was worked out at, read-only and never floating. The two sit one above the other and are meant to differ: the top one is live, the bottom one is history. 245 products were backfilled from the 16 saved worksheets on 22 Sept 2026, each at its own sheet's percentages — several older sheets ran at 15% VAT and shipping between 0 and 50%, and those settings were honoured rather than restated at today's, because the field records what that sheet said. The date is always the SHEET's date on every path, live import and backfill alike; stamping a re-import with today would pair today's date with a months-old rate. source 'landed' now means only “landed and no wholesale price on file”, the one case with nothing to float against, where the settled figure beats showing nothing. A foreign wholesale price copied into costPerItem is therefore always a bug — the two are different quantities in different currencies — and the Product Edit page flags the field when it equals the foreign wholesale price to the cent, because that equality is the signature of an un-converted copy rather than a coincidence.`,
    active: true,
  ...
  {
    id: 'local_suppliers_price_from_inventory',
    name: 'Rule 65 — Local Suppliers Are Priced From Inventory, Not the Import Calculator',
    description: `A supplier who bills us in Rands is local: the goods were never shipped in and never cleared customs, so the costing calculator's import percentages — 25% shipping and 20% customs (Rule 60) — would add a cost we never paid and quote a client roughly 2.17x what we bought at. A local price is therefore NOT derived. Where the supplier's currency is ZAR the calculator is skipped entirely and the estimate is the retail we already sell the item for: pre_order_price ahead of price, exactly the Inventory fallback Rule 61 uses for an item with no wholesale price at all. This is the one case where a known wholesale price does NOT go through the calculator, and it is deliberate: local retail is a figure we set, not one we work out, so R8.00 cost against R15.00 retail must quote R15.00 and never R17.34. A local SKU we have never stocked has no retail to fall back on and reads "On request" for an admin to price by hand, rather than being invented from percentages. The wholesale price is still stored and still shown to admin, because it is what we pay and the margin is read off it; the Supplier Catalogue page shows "From Inventory" in the estimate column for these rows so admin and client can never disagree about the number (Rule 60). Nothing here touches foreign suppliers, whose estimates keep floating with the exchange rate (Rule 63) — a ZAR price has no rate to float against, which is the same reason it needs no conversion.`,
    active: true,
    appliesTo: ['Supplier Pre Orders', 'Supplier Catalogue', 'Suppliers'],
    category: 'Orders',
  },
  ...
  {
    id: 'preorder_dashboard_costing',
    name: 'Rule 67 — Pre-Order Dashboard Costing Is One Formula With Per-Item Percentages',
    description: `Every Pre-Order Dashboard item is costed by ONE formula, with the percentages living on the item rather than in code: wholesale × exchange rate = Cost, Cost × (1 + (Ship % + Customs %)/100) = Landed, Landed × (1 + Markup %) × (1 + VAT %) = Est. Retail Price. The defaults are 25 shipping, 20 customs, 30 markup and 0 VAT. PRE-ORDER ESTIMATES ARE QUOTED WITHOUT VAT on R66Slot, the same as R66Emporium, whose pre-order costing has no VAT step at all — this is the one place the dashboard departs from Rule 60's Supplier Pre Order percentages, which do carry 15% VAT, so the two quote the same goods differently by design and neither is corrected to match the other. The VAT box is kept editable rather than removed so a one-off item can carry VAT without a deploy; an item with VAT on it renders amber like any other off-standard figure. All four are editable per item in the Costing box under Est. Retail Price on the item card, which prints the full working line by line; a value off the standard renders amber with a Reset button so an item priced differently is visible at a glance. THERE ARE NO PER-SUPPLIER FORMULAS. Motorhelix previously had its own hard-coded ×1.20 ×1.30 USD branch that also skipped VAT entirely; it is gone, and a supplier with genuinely different freight is handled by typing different percentages on that item, never by adding a branch back. Pricing Tier 2 shares the same item's percentages: the buy price differs, the cost structure does not. The percentages are stored as shipPct, customsPct, markupPct and vatPct and MUST stay in the PATCH allowedFields whitelist in /api/admin/preorder-dashboard/[id] or they save and silently vanish on reload. Items saved before this rule carry none of them and fall back to the defaults, so nothing needed migrating — an absent percentage means "use the standard", which is why the defaults are never written onto a card just for being opened. THIS FIXED A REAL PRICING FAULT: the dashboard used to read its percentages from a useState default of 20% shipping, fed by /api/admin/costing-settings — a route that was never built. The fetch 404'd on every load and the placeholder stood, so every pre-order was quoted with NO customs in it, roughly 20.83% under on the shipping leg. In practice the damage was worse and less predictable than that: the page ALSO fetched /api/admin/exchange-rates when the route is /api/admin/exchange-rate, singular, so the rate table was empty on every load and calcRetailPrice — which bails when it has no rate for the currency — silently never ran at all for a foreign-currency card, which was 89 of the 92 live cards. Est. Retail was only ever whatever had been typed by hand. BOTH dead fetches are fixed; a fetch on this page that fails must never be swallowed into a default. Est. Retail is DERIVED ON EVERY READ and floats with the rate (Rule 63): the admin grid, the item card, the public pre-order listing and item pages, and the share-card price all price through priceCards in src/lib/preorder-dashboard-price.ts, so the same item cannot be advertised at two different prices. The stored estimatedRetailPrice is a FALLBACK only, used when the exchange-rate feed is unavailable, so a rate problem can stall the figures but can never blank them. A booking stores qty and no price, so existing reservations re-price with the card automatically. THE FLOAT NO LONGER STOPS WHEN THE SHIPMENT LANDS (changed 22 Sept 2026). It keeps moving for as long as a wholesale price exists, because the estimate answers what the NEXT shipment would cost. The settled figure is not lost: it lives on the product Pricing card as Estimate Retail Price from Worksheet, frozen at its sheet's exchange rate. A card only stops floating when it has no wholesale price to float against, or when an admin pins it with the ✏ Manual toggle. The ⚡ Auto / ✏ Manual toggle is the one escape hatch: switching to Manual sets priceManual and pins the typed figure, and the card reopens on Manual so an autosave cannot silently re-float a price that was deliberately pinned. A price is captured the moment an item is sent to a Quote — the line's unitPrice is snapshotted there — so a customer's deposit is taken against the rate on the day they were quoted and never moves afterwards, even while the card keeps floating for everyone else.`,
    active: true,
    appliesTo: ['Pre-Orders', 'Orders', 'Online Store', 'Suppliers'],
    category: 'Orders',
  },
]
