          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              {!productOrgCollapsed && <div className="space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-sm font-medium text-gray-700">Revo Cars Brand Page</label>
                    {carBrands.length > 0 && (
                      <button type="button" onClick={() => setCarBrandPillsHidden(!carBrandPillsHidden)} className="text-gray-400 hover:text-gray-600" title={carBrandPillsHidden ? 'Show selected' : 'Hide selected'}>
                        {carBrandPillsHidden
                          ? <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                          : <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" /></svg>
                        }
                  ...
                  {carBrands.length > 0 && !carBrandPillsHidden && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {carBrands.map(cb => (
                ...
                <div>
                  ...
                  {carTypes.length > 0 && !carTypePillsHidden && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {carTypes.map(ct => (
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              {!sidewaysOrgCollapsed && <div className="space-y-4">

                {/* Sideways Cars Brand Page */}
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-sm font-medium text-gray-700">Sideways Cars Brand Page</label>
                    {sidewaysBrands.length > 0 && (
                      <button type="button" onClick={() => setSidewaysBrandPillsHidden(!sidewaysBrandPillsHidden)} className="text-gray-400 hover:text-gray-600" title={sidewaysBrandPillsHidden ? 'Show selected' : 'Hide selected'}>
                        {sidewaysBrandPillsHidden
                          ? <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                          : <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" /></svg>
                        }
                      </button>
                  ...
                  <div className="relative" ref={sidewaysBrandRef}>
                    <button type="button" onClick={() => setSidewaysBrandDropdownOpen(!sidewaysBrandDropdownOpen)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-left text-sm flex items-center justify-between focus:ring-2 focus:ring-gray-900 focus:border-transparent bg-white">
                    ...
                    {sidewaysBrandDropdownOpen && (
                      <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-56 overflow-y-auto">
                        <label className="flex items-center gap-2 px-3 py-2 hover:bg-gray-50 cursor-pointer border-b border-gray-100">
                          <input type="checkbox" checked={sidewaysBrands.length === 0} onChange={() => setSidewaysBrands([])} className="rounded" />
                          <span className="text-sm text-gray-500 italic">None</span>
                        </label>
                        {sidewaysBrandOptions.map((cb) => (
                          <div key={cb} className="flex items-center gap-1 px-3 py-1.5 hover:bg-gray-50">
                            <label className="flex-1 flex items-center gap-2 cursor-pointer">
                              <input type="checkbox" checked={sidewaysBrands.includes(cb)} onChange={(e) => setSidewaysBrands(e.target.checked ? [...sidewaysBrands, cb] : sidewaysBrands.filter(b => b !== cb))} className="rounded" />
                              <span className="text-sm text-gray-900">{cb}</span>
                            </label>
                        ...
                        <div className="flex items-center gap-1 px-3 py-2 border-t border-gray-100">
                          <input type="text" value={newSidewaysBrandInput} onChange={(e) => setNewSidewaysBrandInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); const val = newSidewaysBrandInput.trim(); if (val && !sidewaysBrandOptions.includes(val)) { setSidewaysBrandOptions(prev => [...prev, val]); setSidewaysBrands(prev => [...prev, val]) }; setNewSidewaysBrandInput('') } }} placeholder="Add brand..." className="flex-1 text-sm border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-gray-900" />
                          <button type="button" onClick={() => { const val = newSidewaysBrandInput.trim(); if (val && !sidewaysBrandOptions.includes(val)) { setSidewaysBrandOptions(prev => [...prev, val]); setSidewaysBrands(prev => [...prev, val]) }; setNewSidewaysBrandInput('') }} className="text-xs px-2 py-1 bg-gray-900 text-white rounded hover:bg-gray-700">+Add</button>
                        </div>
                      </div>
                    )}
                  </div>
                  {sidewaysBrands.length > 0 && !sidewaysBrandPillsHidden && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {sidewaysBrands.map(cb => (
                        <span key={cb} className="inline-flex items-center gap-1 px-2 py-0.5 bg-red-100 text-red-700 text-xs font-semibold rounded-full">
                          {cb}<button type="button" onClick={() => setSidewaysBrands(sidewaysBrands.filter(b => b !== cb))} className="hover:text-red-900">×</button>
                        </span>
                      ))}
                    </div>
                  )}
