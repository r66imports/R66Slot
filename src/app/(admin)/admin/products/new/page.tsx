        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="mt-4">
                <label className="block text-sm font-medium text-gray-700 mb-2">Estimate Retail for Pre Order Dashboard</label>
                <div className="relative">
                  <span className="absolute left-3 top-2 text-gray-500">R</span>
                  <input
                    type="number"
                ...
                <p className="mt-2 text-xs text-gray-500">Pre-order estimate. Derived from the wholesale price at the live rate, excl. VAT — the same figure the Pre-Order Dashboard shows.</p>
              </div>
            </div>
          ...
          <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {allCategories.filter(c => categoryIds.includes(c.id)).map(cat => (
                    <span key={cat.id} className="inline-flex items-center gap-1 bg-blue-100 text-blue-700 text-xs font-medium px-2.5 py-0.5 rounded-full">
                      {cat.name}
                      <button type="button" onClick={() => setCategoryIds(prev => prev.filter(id => id !== cat.id))} className="text-blue-400 hover:text-blue-700 leading-none text-sm">×</button>
                    </span>
                  ))}
                </div>
              ) : (
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-4">
                ...
                {/* Sage Accounts — auto-filled from brand mapping */}
