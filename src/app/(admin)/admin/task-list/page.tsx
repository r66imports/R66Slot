...
export default function TaskListPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const list = Array.isArray(taskData) ? taskData.map(normalize) : []
      setTasks(list)
      setSuppliers(Array.isArray(supplierData) ? supplierData : [])
      setLoading(false)
      // Auto-sync after tasks are loaded so we can dedup
      autoSyncNoImageProducts(list)
    })
  ...
  async function createTask(title: string, date: string, priority: TaskPriority) {
      ...
      <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-black mt-0.5">
            Tasks grouped by date. Completed tasks turn green and disappear after 5 days.
          </p>
        </div>
            ...
            dateLines.length === 0 ? (
              <div className="text-center py-16 px-4">
                ...
                <p className="text-sm text-black mt-1">
                  {!search && !selectedSupplier && !priorityFilter
                    ? 'Use "+ Create Task" to add one. Products missing images are added automatically.'
                      ...
                      <>
                        ...
                        {line.items.map(task => (
                          <TaskRow
                            ...
                            onSaveTitle={title => saveTitle(task.id, title)}
