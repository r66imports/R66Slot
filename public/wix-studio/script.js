...
(function(){
  ...
  function init(){
    ...
    window.setBreakpoint = function(view, el){ document.querySelectorAll('.bp-btn').forEach(b=>b.classList.remove('active')); if(el) el.classList.add('active'); canvas.getObjects().forEach(obj=>{ if(!obj.responsiveData) obj.responsiveData = {}; obj.responsiveData[activeView] = { left: obj.left, top: obj.top, scaleX: obj.scaleX, scaleY: obj.scaleY } }); activeView = view; const newWidth = viewWidths[view]; canvas.setWidth(newWidth); const scrollArea = document.querySelector('.canvas-scroll'); if(scrollArea) scrollArea.style.width = newWidth + 'px'; canvas.getObjects().forEach(obj=>{ if(obj.responsiveData && obj.responsiveData[view]){ obj.set({ left: obj.responsiveData[view].left, top: obj.responsiveData[view].top, scaleX: obj.responsiveData[view].scaleX, scaleY: obj.responsiveData[view].scaleY }); } obj.setCoords(); }); canvas.renderAll(); }

    window.spawn = function(type){ if(type==='text'){ const txt = new fabric.Textbox('Edit me',{ left:100, top:100, fontSize:28, fill:'#fff' }); canvas.add(txt).setActiveObject(txt); } else if(type==='img'){ document.getElementById('image-upload').click(); } else if(type==='box'){ const rect = new fabric.Rect({ left:120, top:120, width:200, height:120, fill:'#0078d4' }); canvas.add(rect).setActiveObject(rect); } };

    window.addContainer = function(){ const rect = new fabric.Rect({ left:140, top:140, width:400, height:200, fill:'transparent', stroke:'#777', strokeDashArray:[4,2], hasBorders:false }); rect.isContainer = true; canvas.add(rect).setActiveObject(rect); };

    window.handleImageUpload = function(e){ const file = e.target.files && e.target.files[0]; if(!file) return; const reader = new FileReader(); reader.onload = function(ev){ fabric.Image.fromURL(ev.target.result, function(img){ img.set({ left:80, top:80, scaleX:0.5, scaleY:0.5 }); canvas.add(img).setActiveObject(img); }); }; reader.readAsDataURL(file); };
